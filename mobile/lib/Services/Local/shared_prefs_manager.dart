...
//set data into shared preferences like this
  ...
  Future<dynamic> getEvent() async {
    SharedPreferences preferences = await SharedPreferences.getInstance();
    var dataList = preferences.getString('events');
    return dataList;
  }

  setEvent(EventResponseModel event) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    List<EventResponseModel> ev = [];

    var oldData = await prefs.getString("events");
    if (oldData == null) {
      ev.add(event);
    }else{
      var convertedData = json.decode(oldData);
      convertedData.add(event);
    }

    prefs.setString("events", json.encode(ev));
  }
