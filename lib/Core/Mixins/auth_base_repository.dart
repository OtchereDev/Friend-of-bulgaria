...
import 'package:hackaton/Utils/Dialogs/dialog_utils.dart';
...
mixin AuthBaseRepository {
  ...
  Future<Map<String, String>> setHeaders() async {
   ...
   Future<Map<String, String>> setHeaderNoContent() async {
    token = await SharedPrefsManager().getAuthToken();
    return {
      'Authorization': 'Bearer $token',
      'Accept': 'application/json',
  ...
  Future<http.Response?> post(context,
      {required String url, dynamic data, bool useContentType = false}) async {
    Map<String, String> headers = await setHeaders();
    Map<String, String> headerNoContent = await setHeaderNoContent();
    ...
    try {
      ...
      response = await http
          .post(Uri.parse(url), headers:useContentType? headerNoContent: headers , body: data)
          .timeout(const Duration(seconds: 60));
